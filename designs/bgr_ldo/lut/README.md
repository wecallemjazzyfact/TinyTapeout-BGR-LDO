...
> ### ★ 삼극관 R_on 사이징은 LUT 조회만으로 확정하지 않는다 — geometry 명시 .op 로 최종 확정한다
>
...
> 포화영역(thick-ox 3소자 회로검증, ID 오차 $\le +1.05\%$)에서는 채널저항이 커서 이 직렬성분이 무시됐으나, 삼극관($V_{DS}$ 작음, 채널저항 자체가 작음)에서는 전체의 20~30%를 차지할 수 있다 — 동작영역에 따라 민감도가 근본적으로 다르므로, 포화 검증이 삼극관을 담보하지 않는다.
>
...
>
> **규율**: 삼극관으로 동작하는 스위치(트림 스위치, 부하 싱크 등)의 $R_{on}$은 (1) LUT로 1차 사이징 $\rightarrow$ (2) 최종 geometry 명시 `.op`로 확정. 두 값 차이가 10% 이상이면 실측값을 채택한다.
