...
* **5-bit 저항 트림 회로 내장:** 몬테카를로 보수 기준 3-sigma 산포($\pm 7.5\%$, pooled $\sigma \approx 2.30\sim2.5\%$, MC mean $1.19237\,\text{V}$)를 완전 커버하고 8 ppm/°C 초정밀 성능을 튜닝하기 위해, $R_2$ 고정부 38.4유닛 + 트림 스팬 6.2유닛(5-bit LSB 0.2유닛 = $590\,\Omega$, 스텝 $0.48\%$, 중심 코드에서 $1.20\,\text{V}$ 산출) 비대칭 트림 사양을 적용함. 트림 후 잔여 오차는 $\pm 0.24\%$.
* **최소 동작 전원 전압 ($V_{min}$ 실측):** tt $2.190\,\text{V}$ / ss−40 $2.380\,\text{V}$ / ff125 $2.050\,\text{V}$ (양방향 동일, 히스테리시스 없음). 최소 동작 전원 $VAPWR = 3.0\,\text{V}$ 대비 최악 조건 마진 **`+620 mV`** 확보.
...
| $I_Q$ (BGR only) | $< 60\,\text{µA}$ | 59.73 µA @tt (실측치, 코어 기준) [1] |
...
`[1] 개정 사유: TC 최적화 분석 결과 브랜치당 ~10µA의 바이어스 전류가 요구됨에 따라 코어2+출력1+bias1 분기 합산 55µA 소모에, Line Reg 개선을 위한 cascode bias 신규 분기(+1.2µA) 및 core 면적 스케일링에 따른 기생 증가(+3.53µA)가 추가되어 총 59.73µA가 소모됨. IB_EA 탭 전류 2.564 µA는 EA 계정으로 이관되며 전체 물리 소모 전류는 62.3 µA.`
...
* **면적 스케일링 처방 및 탭 추가 후 산포 재측정:**
  ...
  * $W/L$ 종횡비를 유지함으로써 $V_{ov}$, 바이어스 전류 및 동작점을 보존하면서 오프셋 산포 성능만 안전하게 개선함.
  * **IB_EA 탭 추가 후 MC 재측정 ($N=100$, MM+PR)**: $\sigma(V_{ref}) = 2.38\%$, $\sigma(IB\_EA) = 2.53\%$, $\text{corr}(V_{ref}, IB\_EA) = +0.692$.
  * **통계 판정 각주**: 탭 추가 전 $\sigma = 2.21\%$와의 차이는 표본 표준오차($\pm 7.1\%$) 범위 이내로 F-검정(F-test)으로 유의미한 차이를 나타내지 않음 (기각 불가). 설계 기준으로는 pooled $\sigma \approx 2.30\%$ (보수적 적용 시 $2.5\%$)를 사용함.
...
* [x] **[Step 2.8] BGR 몬테카를로 재실행 및 트림 설계 확정:**
  * 면적 스케일업 및 IB_EA 탭 적용 후 몬테카를로 산포 재실측 완료 ($\sigma = 2.38\%$, 보수 기준 $\sigma \approx 2.5\%$).
  * 3-sigma 산포 $\pm 7.5\%$ 커버를 위한 5-bit 비대칭 저항 트림($R_2$ 고정 38.4유닛 + 트림 스팬 6.2유닛, LSB 0.2유닛 = $590\,\Omega$, 스텝 $0.48\%$, 중심 코드 $1.20\,\text{V}$) 사양 확정 완료. 잔여 오차 $\pm 0.24\%$.
