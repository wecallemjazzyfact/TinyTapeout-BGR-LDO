...
* [x] **[Step 2.8] BGR 몬테카를로 재실행 및 트림 설계 확정:**
  ...
  * **★ 물리적 실현 제약 (신규 발견):**
    5-bit 단순 분할 시 LSB $= 5.51 / 31 = 0.178\,\text{유닛} = 524\,\Omega$입니다. 그러나 `res_high_po_0p69` 소자의 최소 실현 가능 저항은 단자 2개의 접촉 헤드 저항 $\approx 780\,\Omega$ ($390\,\Omega/\text{terminal}$)입니다. 즉 LSB 저항이 물리 최소 한계($780\,\Omega$)보다 작아 $R_2$ 단순 직렬 폴리 세그먼트로는 구현이 불가능합니다.
  * **해결 후보 2안 (트림 회로 구현 단계에서 확정):**
    * **(a) 4-bit 채택안**: LSB $= 5.51 / 15 = 0.367\,\text{유닛} = 1083\,\Omega$ (헤드 $780\,\Omega$ + 본체 $303\,\Omega$). LSB 스텝 $0.89\%$, 잔여 오차 $\pm 0.45\% \rightarrow V_{out} \pm 8\,\text{mV}$ (스펙 $\pm 2\%$ 내 수용). `ui_in` 핀 1개 절약 효과.
    * **(b) 광폭 폴리 저항 하이브리드안**: 트림 세그먼트 구간만 폭이 넓은 폴리 저항(`res_high_po_2p85` 또는 `5p73`) 적용. `5p73` 사용 시 헤드 저항 $\approx 47\,\Omega/\text{단자}$로 감소하여 LSB $524\,\Omega$ 물리 구현 가능. 5-bit 유지 및 잔여 오차 $\pm 0.22\%$ 확보 (단, 헤드 면적비 변경에 따른 TC 기여도 재검토 필요).
