...
### 2.2.1. BGR TRIM 극성 및 소프트웨어 코드 반전 폐기 (LDO top 하드웨어 반전)
★ **소프트웨어 코드 반전 지시 취소** (REPLY_LDO_TO_BGR_STARTUP_CLAMP §5).
LDO top 에 인버터 4쌍(thin-ox nfet W2/L0.15 + pfet W4/L0.15, VDPWR 급전)이 들어가 하드웨어로 반전합니다. 소프트웨어에서 추가 반전 금지 — 이중 반전 시 ui_in=0000 이 다시 최대 전압이 되어 위험합니다.
...
### 3.3 레이아웃 인계 (LVS 결과 및 더미 최종값)

* **LVS 결과**: **Circuits match uniquely** (44/44 소자, 32/32 넷)
* **더미 저항 최종 L (magic 추출값 반영)**:
  * XXDUM_RA1, XXDUM_RA2 : **L=31.49** (구 30.23)
  * XXDUM_RB1, XXDUM_RB2 : **L=58.41** (구 57.15)
* **정정 사유**: 더미는 X Repeat=2(굴곡 1개)인데 N=8 보정식을 쓴 초기값이라 굴곡 고정항 +1.26 µm 가 누락됐다. 실소자는 N=8 보정식이 이미 맞아 무관.
* **전기적 무영향 실측**: 정정 전후 V_ref/IB_EA/I_Q 6자리 불변.
...
### BGR MC 5회 종합 및 Pooled $\sigma$ 유지 근거

BGR MC 5회 실측 결과 (전부 $n=100$, MM+PR, tt/27°C):
- 면적 스케일링 후: $\sigma = 2.21\%$
- IB_EA 탭 추가 후: $\sigma = 2.38\%$
- 벌크 수정 후: $\sigma = 2.06\%$
- 트림 스위치 적용 후: $\sigma = 2.38\%$
- **R6/R7 2분할 후**: **$\sigma = 2.02\%$**

★ **분석 판정**: 다섯 회차 전부 동일 분포입니다. $n=100$에서 $\sigma$ 추정량의 표준오차가 $\pm 0.16\%p$이며 $2.02 \sim 2.38\%$는 그 표준오차 범위 안입니다. **분할이 $\sigma$를 개선한 것이 아닙니다** — 랜덤 미스매치는 총 면적에 의존하고, 분할은 면적 합을 바꾸지 않습니다. 분할이 개선하는 것은 계통 구배(공통중심 배치)뿐입니다.
$
ightarrow$ 설계 기준 **pooled $\sigma = 2.22\%$, $\pm 3\sigma = \pm 6.66\%$**를 유지합니다. 트림 창 $\pm 7.2\%$가 커버합니다.
...
### 2.5. 폐기 수치 명시 (인용 금지 목록)

아래 항목들은 설계 검증 과정에서 폐기된 수치들이므로 향후 문서 작성 및 계산 시 인용을 엄금합니다.

1. **유닛 직렬 TC 탐색 결과 전체** (8:55 $7.0\,	ext{ppm/°C}$ / 10:69 $11.2\,	ext{ppm/°C}$ / 블록 $14.8\,	ext{ppm/°C}$ 등):
   - 유닛 셀 어레이 전제였으나 서펜타인 배치를 채택함에 따라 무효화됨.
2. **초기 직렬 시뮬레이션 $99 \sim 147\,	ext{ppm/°C}$**:
   - 노드 충돌 버그에 의한 실측 아티팩트이므로 절대 인용 금지.
3. **"유닛 실효 면적 $5.85\,\mu	ext{m}^2$"**:
   - 레이아웃 측 폐기 통보 (정정 실효 면적 $6.129\,\mu	ext{m}^2$).
4. **폴리 TC1 단독값 $+514\,	ext{ppm/°C}$**:
   - 컨택 헤드가 혼입된 실효값이었으므로, 헤드와 몸통을 분리한 **2성분 모델**(몸통 $545.3\,	ext{ppm/°C}$, 헤드 $120.6\,	ext{ppm/°C}$)로 대체됨.
...
## 🛠️ 10. PEX 직접 스윕 기법 (Direct PEX Parameter Sweep Technique)
...
* **PEX 직접 스윕**: 원본 값에서 $10.85\,\text{ppm}/^\circ\text{C}$로 PEX 실측을 정확히 재현.
* **최적점 확정**: $L=34.25\,\mu\text{m}$에서 최소점 $\mathbf{6.13\,\text{ppm}/^\circ\text{C}}$ 확정 $\rightarrow$ 레이아웃 재작업 **1회로 종료**.

### 10.4. 한계 및 고려 사항
* 소자 치수 변경에 따른 배선 길이의 미세 변화(수십 $\text{m}\Omega$)는 반영되지 않으나, 영향이 미미하여 무시 가능하다.
* 소자가 기본 저항(`R`)으로 완전 분해(Flatten)된 경우, 소자 몸통 성분만 선별하여 스케일링해야 하므로 가공 방식이 복잡해진다.

---

## 📐 11. 레이아웃 인계 규율 (LVS / 더미 / DRC / PEX)

### 11.1. LVS 골든 넷리스트 생성 규율
* **`mult=1` 자동 제거 필수**: xschem에서 "top level is a .subckt" 체크 후 추출 시, 아래 `sed` 명령을 실행하여 `mult=1` 속성을 전면 제거해야 한다:
  ```bash
  sed 's/\(res_high_po_[0-9]*p[0-9]* L=[0-9.]*\) mult=1/\1/g' bgr_core.spice > bgr_core_lvs.spice
  ```
  * **이유**: Netgen이 "No property mult found" property error를 낸다. xschem 심볼 템플릿에 `mult=1`이 고정되어 있어 인스턴스에서 삭제해도 재추출 시 복원됨.
  * ★ 넷리스트 재생성 시 반드시 `sed` 명령을 재실행할 것.
  * ★ TB(Testbench) 파일은 `.subckt` 체크 해제, 서브회로 셀 파일은 체크하여 목적에 맞게 추출.
  * ★ Netgen의 property cutoff 오차 허용 한계는 **1%**이다. 1% 미만의 치수 차이(예: $R_1$ $34.45\,\mu\text{m}$ vs $34.29\,\mu\text{m} = 0.46\%$ 차이)는 LVS를 그냥 통과하므로, **LVS 통과가 시뮬레이션 일치를 보장하지 않는다.** 스케매틱 변경 시 골든 넷리스트를 반드시 갱신해야 한다.

### 11.2. 더미 소자(Dummy Devices) 배치 규율
* **전위차 0 보장**: 더미 소자의 전 단자를 동일 전원/접지에 단락시켜 전위차를 0으로 만든다 ➔ 전류 소모 및 접합 용량 기여가 수학적으로 0임이 보장된다 (실측: 더미 추가 전후 $V_{ref}, I_Q, IB_{EA}$ 6자리 동일).
* **스키매틱 명시 목적**: 더미 개수를 정량 문서화하고, LVS에서 구조를 검증하며, PEX 추출 시의 넷리스트 불일치를 방지함.
* **규약**: `XDUM_` 접두사 사용 / 더미 저항은 양단을 `VGND`에 단락 (플로팅 엄금) / 개수가 많을 경우 `m=N` 단일 인스턴스로 작성 가능 (Netgen 병렬 자동 병합).

### 11.3. MOS `nf` (Fingers) 제약
* `nf >= 2` 배치는 물리적으로 불가능하다 (확산 스트립 폭 $0.23\,\mu\text{m} < \text{via1 } 0.26\,\mu\text{m}$, 게이트 컨택 이격 여유 $0.025\,\mu\text{m} < \text{met1.2 } 0.14\,\mu\text{m}$).
* **규율**: `m=N` 사양은 **`nf=1` 유닛 소자 N개를 개별로 배치**하는 방식을 적용하며, 스키매틱에서도 `nf=1, m=N` 형태를 엄격히 유지한다.

### 11.4. Magic DRC 모드 규율
* ★ 반드시 **`drc style drc(full)` + `drc euclidean on`** 모드로 검증한다.
* 기본 모드인 `drc(fast)`에서는 Latch-up(`LU.2`, `LU.3`), `diff/tap.18,20`, `rpm.1a` 등이 검출되지 않는다. TT 공식 Precheck는 KLayout FEOL 기준이며 예외 웨이버가 없다.
* *실측 사례*: `fast` 모드에서 clean 판정을 받았던 BGR 코어에서 `full` 모드 전환 후 `LU.2` 88건, `LU.3` 32건 등이 검출되어 전면 수정을 수행함.

### 11.5. PEX 추출 및 산출물 처리 주의 사항
1. **노드명 치환 시 저항/용량 값 필드 보호**: 점 노드 치환 스크립트 실행 시 저항/용량 값 필드를 절대 건드리면 안 된다 (`10079.4 -> 10079_4` 치환 시 회로 특성 전면 파괴).
   * *실측 사고*: $I_Q$가 정상의 3배($190.5\,\mu\text{A}$)로 튐. 검증 명령 `grep -cE '^[RC][0-9]+ [^ ]+ [^ ]+ [0-9]+_[0-9]'` 실행 결과가 0이어야 함.
2. **$I_Q$ 1차 관문 검증**: PEX 넷리스트 정상 여부의 가장 빠른 검증 지표는 소모 전류 $I_Q$이다. 스키매틱 대비 $\pm 10\%$ 범위를 벗어나면 PEX 넷리스트 구성을 의심해야 한다.
3. **★ 래퍼 포트 순서 수동/자동 대조**: PEX 재추출 시 `.subckt` 라인의 핀 순서가 변경될 수 있다. 순서가 어긋나도 ngspice 시뮬레이션은 에러 없이 오동작 결과만 낸다 (실측 사례: `IB_EA` 핀이 4번째에서 맨 뒤로 이동했는데 래퍼가 옛 순서를 유지하여 결선이 한 칸씩 밀림 ➔ $V_{ref}=1.112\,\text{V}$, $i(vload)=0$ 산출).
4. **Flatten 필수**: 계층(Hierarchical) 상태에서의 R+C 추출은 의미 있는 결과를 보장하지 않으므로 R+C 추출 시 `flatten` 작업이 필수적이다 (Magic 개발자 확인).
5. **Xschem 심볼 캐시 방지**: 하위 셀 수정 후 상위 디렉터리에서 스키매틱을 열면 이전 심볼이 로드될 수 있으므로, Xschem을 완전히 종료 후 재기동하거나 하위 셀 디렉터리에서 먼저 오픈한다.

---

## 🚫 12. 절대 인용 금지 목록 (Obsolete & Invalid Data Ledger)

아래 항목들은 과거 잘못된 전제, 노드 충돌 버그, 파싱 오류로 산출된 폐기 수치들이므로 **이후 문서 및 설계 판단 시 절대 인용을 금지**합니다.

1. **유닛 직렬 TC 탐색 결과 전체** (`8:55` 7.0 ppm, `10:69` 11.2 ppm, 블록 14.8 ppm 등): 유닛 셀 어레이 배치를 전제로 수행되었으나, 최종 레이아웃이 서펜타인(serpentine) 구조로 확정됨에 따라 전면 무효화됨.
2. **초기 직렬 시뮬레이션 TC 수치 ($99 \sim 147\,\text{ppm}/^\circ\text{C}$)**: 스파이스 연속행 파싱 실패에 따른 노드 충돌 버그 산물.
3. **손상된 PEX 시뮬레이션 결과** ($I_Q = 190.5\,\mu\text{A}$, $\text{TC} = 170.7\,\text{ppm}/^\circ\text{C}$, $V_{ref} = 1.219814\,\text{V}$): 점 노드 치환 스크립트가 저항/용량 값의 소수점까지 밑줄로 바꾼 버그의 결과.
4. **"유닛 실효 면적 $5.85\,\mu\text{m}^2$"**: 레이아웃 실측 폐기 통보 수치 (실측 정정치: $6.129\,\mu\text{m}^2$).
5. **폴리 TC1 단독값 $+514\,\text{ppm}/^\circ\text{C}$**: 헤드 저항 성분이 혼입된 실효값임. 2성분 모델($\text{TC}_{\text{body}}=545.3$, $\text{TC}_{\text{head}}=120.6$)로 대체함.
6. **코너 증분 1차 수식 ($l = N \cdot L + (N-1) \times 0.1927\,\mu\text{m}$)**: N에 따라 굴곡 증분의 부호가 변동함이 밝혀져 폐기함.
7. **해석적 배선 저항 예측값** (P1 27.24 Ω, P2 12.53 Ω 등): 폴리 컨택 저항($50.67\,\Omega$/단자)이 누락되어 과소평가된 수치임. PEX `extresist` 추출값이 정본임.
8. **BGR 면적 출처 불분명 언급** ("$60 \times 90 = 5,400\,\mu\text{m}^2$", "$\sim 4,300\,\mu\text{m}^2$"): 출처 미상의 수치임. Bottom-up 실측 견적치($6,600 \sim 7,400\,\mu\text{m}^2$)가 정본임.
9. **$R_1 = 34.45\,\mu\text{m}$ 시절 측정값 전체** ($\text{TC} = 10.85\,\text{ppm}/^\circ\text{C}$, $V_{ref} = 1.20471\,\text{V}$ 등): $L = 34.29\,\mu\text{m}$ PEX 최적화 수치($\text{TC} = 6.70\,\text{ppm}/^\circ\text{C}$, $V_{ref} = 1.20682\,\text{V}$)로 전면 대체됨.
