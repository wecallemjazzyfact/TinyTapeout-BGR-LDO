...
*   **■ 온도 곡률 — 기존 "tc2 보상" 서술 폐기**:
    *   ablation 분해 결과 (`bgr_curvature_origin.md`):
        *   2차 곡률 $a_2 = -21.19\,\text{mV}$는 TNF2($-95.25$)와 IKF/TIKF1($+80.86$)의 상쇄 잔차임.
        *   3차 곡률 $a_3 = +102.18\,\text{mV}$는 IKF/TIKF1 항($+102.67$)이 거의 전부임.
        *   저항 기여는 $a_2$의 9%, $a_3$의 0.8%에 그침.
    *   ★ 저항 기여 메커니즘도 `tc2`가 아니라 **head/body의 `tc1` 차이로 인한 저항비 드리프트**임. 모든 저항의 `tc`가 동일했다면 비에서 완전 상쇄되어 기여가 0이었을 것임.
    *   ※ `res_high_po`의 `tc2` 실측은 $+1.20\times 10^{-6}/^\circ\text{C}^2$ ($=+1.20\,\text{ppm}/^\circ\text{C}^2$)로 기존 값과 근사하나, 그것이 곡률을 보상한다는 인과는 성립하지 않음.

### 3.1.1. 폴리 저항 2성분 모델 (헤드 TC 2성분 모델)

■ 구조
  res_high_po_0p69 는 몸통(폴리)과 헤드(양단 컨택 단자부)의 직렬이다.
  - $R(L) = 470.976 \times L + 525.70\ [\Omega]$ (기존, 헤드 262.85 Ω/단자 × 2)
  - $R(L) = 471.698 \times L + 525.88\ [\Omega]$ (2026-08 재측정, res_nom / 30 °C)
  → 0.15 % 차이. (측정 조건·추출 방식 확인 병기)
...
■ 2성분 모델 (온도 기준별 병기)
  - **30 °C 기준**: $\text{TC}_{\text{body}} = 545.3\,\text{ppm}/^\circ\text{C}, \text{TC}_{\text{head}} = 120.6\,\text{ppm}/^\circ\text{C}$ (비 4.52) ← 기존
  - **-37.5 °C 국소**: $\text{TC}_{\text{body}} = 359.8\,\text{ppm}/^\circ\text{C}, \text{TC}_{\text{head}} = 79.7\,\text{ppm}/^\circ\text{C}$ (비 4.51) ← 재측정
  ★ 기준 온도가 다르므로 절대값 직접 비교 불가. tc2 때문에 국소 tc는 온도에 따라 변합니다. 비가 일치하여 구조는 확인됩니다.
  정리형: $\text{TC}_{\text{eff}} = 545.3 - 223,265 \cdot n / R$ (n = 헤드 쌍 수)
...
■ 최종 확정 저항 L (magic gencell N=8 실측값)
  - **XR1: L = 34.2900 µm** (L_col 4.320, TC 최적화 반영)
    ★ $L_{\text{col}}$은 $0.01\,\mu\text{m}$ 배수만 사용 가능. $4.315\,\mu\text{m}$($0.005\,\mu\text{m}$ 홀수배)는 magic이 폴리를 23 타일로 쪼개 추출 $l$이 $35.19\,\mu\text{m}$로 튐.
    $0.01\,\mu\text{m}$ 배수 구간에서 $l = 8 \cdot L_{\text{col}} - 0.27$이 정확히 성립함.
    실측: $4.300 \rightarrow 34.13$ / $4.320 \rightarrow 34.29$ / $4.330 \rightarrow 34.37$ / $4.340 \rightarrow 34.45\,\mu\text{m}$
...
#### 저항 코너 구조 및 파라미터 파일 3종 (Sky130A 정본)

> **★ 기존 서술 오류 2건 정정**:
> 1. `res_high_po_rs = {325.0 + corner_factor × 45.0}` ➜ **`res_nom`은 `× 0.0`.** `× 45.0`은 `res_high` 파일의 수식임.
> 2. "저항 코너는 독립 설정되지 않고 트랜지스터 코너에 종속" ➜ **완전히 독립.** `ss_ll`, `ff_hh` 같은 조합 섹션이 명시적으로 존재함.
> 3. **`corner_factor`는 전 섹션에서 `1`로 고정**이며, 코너를 결정하는 것은 **어느 `parameters_*.spice`를 include하는가**입니다.

##### 1. 저항 파라미터 파일 3종 (실측)
`sw_sky130_fd_pr__res_high_po_rs`와 `sw_poly_head_res` 기준:

| 파일 | 시트 수식 | 시트 값 | head 배율 수식 | head 배율 | Body 변동률 | Head 변동률 |
| :--- | :--- | ---: | :--- | ---: | ---: | ---: |
| `parameters_res_nom.spice` | `{325.0 + cf*0.0}` | **325.0** Ω/□ | `{1.0 + cf*0.0}` | **1.000** | 기준 | 기준 |
| `parameters_res_low.spice` | `{325.0 + cf*(-48.0)}` | **277.0** Ω/□ | `{1.0 + cf*(-0.125)}` | **0.875** | **−14.77 %** | **−12.5 %** |
| `parameters_res_high.spice` | `{325.0 + cf*45.0}` | **370.0** Ω/□ | `{1.0 + cf*0.125}` | **1.125** | **+13.85 %** | **+12.5 %** |

*(`cf` = `corner_factor` = 1)*

##### 2. 섹션 ➜ 파일 매핑 및 MC 스위치 규칙 (전수)
*   **매핑 규칙**:
    *   기본 접미사 없음 (`tt`, `sf`, `ff`, `ss`, `fs`, `mc`): **`res_nom`** + `cap_nom`
    *   `_ll`, `_lh`, `ss_ll`, `ss_lh`, `sf_ll`, `fs_ll`, `fs_lh`, `ff_ll`, `ff_lh`: **`res_low`** + `cap_low`
    *   `_hh`, `_hl`, `ss_hl`, `ss_hh`, `sf_hl`, `sf_hh`, `fs_hl`, `fs_hh`, `ff_hl`, `ff_hh`: **`res_high`** + `cap_high`
    *   *(각 섹션의 `_mm` 변형도 매핑 동일: `tt_mm` ➜ `res_nom`, `ll_mm` ➜ `res_low`)*
*   **MC 스위치 동작**:
    *   기본 섹션: `MC_MM_SWITCH = 0`, `MC_PR_SWITCH = 0`
    *   `_mm` 변형 섹션: `MC_MM_SWITCH = 1`, `MC_PR_SWITCH = 0` (**Mismatch만 켬**)
    *   **`_mm` 섹션(PR=0)에서는 저항 산포가 전혀 없습니다.** 저항 공정 산포(PR, 시트 $\sigma=3.5\%$, Head $\sigma=2.5\%$)를 켜려면 `.lib` 뒤에 수동 주입해야 합니다:
        ```spice
...
*   저항 산포를 보려면 **조합 섹션(`ff_hh`, `ss_ll`, `sf_hl` 등)**을 사용하거나 **PR MC (`MC_PR_SWITCH=1`)**를 켜야 합니다.

##### 4. BGR에 대한 함의
*   **1차 (비 상쇄)**: $V_{\text{ref}} = \frac{R_2}{R_7}V_{BE1} + \frac{R_2}{R_1}\Delta V_{BE}$ 에서 시트 저항은 1차 상쇄됨. FET 코너 간에는 시트가 아예 불변이므로 상쇄를 논할 필요조차 없음.
*   **★ 2차 (Head/Body 변동률 차이)**:
    *   Body와 Head의 변동률 차이(low 기준 차이 $+2.7\%\text{p}$, high 기준 $-1.4\%\text{p}$)로 인해 head 분율이 저항마다 달라 유효 저항비가 코너에 따라 미세 드리프트(약 $0.07\%$).
    *   Head 분율 (30 °C, `res_nom` 기준, $K = 471.698\,\Omega/\mu\text{m}$, $H = 525.88\,\Omega$):
        *   $R_1$ ($34.29\,\mu\text{m}$): **`3.149 %`** (가장 민감, 7배 큼)
        *   $R_7$ ($\times 2$ 직렬, $121.73\,\mu\text{m}$): **`0.908 %`**
        *   $R_2$ ($247.37\,\mu\text{m}$): **`0.449 %`**
    *   **추정 드리프트**: $\frac{\Delta k_2}{k_2} \approx (0.03149 - 0.00449) \times \left(\frac{0.875}{0.8523} - 1\right) \approx 7.2 \times 10^{-4}$ (약 $0.07\%$, 트림 LSB $0.89\%$의 $8\%$).
    *   **★ TC 인용 주의**: `ll_mm`은 저항 $-14.8\%$ + head 배율 $0.875$로 head/body 비가 바뀌어 TC가 달라집니다. startup 판정에는 무관하나, **TC 수치를 `ll` 코너에서 가져오면 안 됩니다.**

##### 5. Head / Body 실측 온도계수 정본
*   **`tc1sky130_fd_pr__res_generic_pobody = -1.47e-3` (`models_global.spice` L85) 오인용 주의**: 실측 TC는 **양수(+)**이며, 해당 파라미터는 `res_high_po`에 적용되지 않습니다.
*   6점 실측 ($L = 1, 10, 34.29, 50, 121.73, 243.46\,\mu\text{m}$, `tt`, $-40 \sim 125\,^\circ\text{C}$):
    $$\text{tc}_{\text{eff}}(R) = \text{TC}_{\text{body}} - \frac{(\text{TC}_{\text{body}}-\text{TC}_{\text{head}})R_{\text{head}}}{R}$$
    *   $-37.5\,^\circ\text{C}$ 국소 기준: $\text{TC}_{\text{body}} = 359.8\,\text{ppm/}^\circ\text{C}$, $\text{TC}_{\text{head}} = 79.7\,\text{ppm/}^\circ\text{C}$ (비 **4.51**, 피팅 RMS $0.000\,\text{ppm/}^\circ\text{C}$)
    *   $30\,^\circ\text{C}$ 환산 기준: $\text{TC}_{\text{body}} = 545.3\,\text{ppm/}^\circ\text{C}$, $\text{TC}_{\text{head}} = 120.6\,\text{ppm/}^\circ\text{C}$ (비 **4.52**)
...
PYEOF'
```
> ★ `.endl`까지만 읽어야 합니다. `sed n+20` 방식은 인접 섹션을 긁어옵니다.
...
(최소 4점, 권장 6점 다점 실측 필수)

---

## 부록 B. 이 문서가 나오게 된 경위 — 오류 연쇄 및 교훈

| # | 오류 | 발견 경로 | 교훈 |
| ---: | :--- | :--- | :--- |
| 1 | 저항 2점 tc를 "0.2 % 일치"로 **계산 없이 단정** | 기존 2성분 모델과 충돌 지적 | "일치/불일치" 판정은 **반드시 수치를 출력**한다. |
| 2 | 그 위에서 "폴리 TC 기여 정확히 0 %" 결론 ➜ 작업 가설 **오기각** | 〃 | 기존 결론과 충돌하면 **양쪽을 같은 방법으로 재측정**한다. |
| 3 | `sed n+20`으로 섹션 덤프 ➜ 인접 섹션 혼입 ➜ `tt_mm`·`ll_mm` 동일 오판 | `corner_factor` 값이 전 섹션 동일해 의심 | 섹션 덤프는 **`.endl` 경계를 엄격히 지킨다.** |
| 4 | `03_PDK_환경`의 `× 45.0` 수식을 그대로 신뢰 | `res_nom` 직접 grep | PDK 파라미터를 **이름으로 추정하지 않고 직접 시뮬/grep해 실측**한다. |
| 5 | Antigravity의 `370(SS)/277(FF)` 라벨 오류 | 파일별 대조 | 코너명이 아니라 **파라미터 파일 이름**을 기준으로 대조한다. |
