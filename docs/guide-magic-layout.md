---
title: Laying Out Standard Cells with Magic VLSI :: Quicker, easier and cheaper to make your own chip!
source: https://tinytapeout.com/guides/laying-standard-cells-with-magic/
snapshot: 2026-07-10
---

[Tiny Tapeout](/) > [Guides](/guides/) > Laying Out Standard Cells with Magic VLSI

# Laying Out Standard Cells with Magic VLSI
...
This may be useful if you have been following the [local hardening guide](/guides/local-hardening/) and have already set those
variables.
...
You can move an already placed cell by first hovering over it and pressing `I`, and then moving your cursor
elsewhere and pressing `M`. It will move the bottom-left of the cell to where your cursor is.
...
#### 2 DFFs with a tap cell in the middle, grid overlay

#### 2 DFFs with a tap cell in the middle, no grid overlay
...
To show the contents of a cell, move the mouse over it, press `I` and then press `X` (for “expand”). To hide the contents
of a cell, press `Shift` + `X`.
...
vertically, select the instance (by moving the mouse over it and pressing `I`), then press `Shift` + `F`.

The cells should overlap each other, as shown in the figure below.

#### A tap cell overlapping a DFF cell
